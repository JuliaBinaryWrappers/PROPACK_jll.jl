# Autogenerated wrapper script for PROPACK_jll for x86_64-apple-darwin14-libgfortran5
...
const libcpropack_splitpath = ["lib", "libcpropack.dylib"]
...
const libcpropack = "@rpath/libcpropack.dylib"


# Relative path to `libzpropack`
const libzpropack_splitpath = ["lib", "libzpropack.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libzpropack = "@rpath/libzpropack.dylib"


# Relative path to `libdpropack`
const libdpropack_splitpath = ["lib", "libdpropack.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libdpropack = "@rpath/libdpropack.dylib"


# Relative path to `libspropack`
const libspropack_splitpath = ["lib", "libspropack.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libspropack = "@rpath/libspropack.dylib"
